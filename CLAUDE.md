...
- `src/web_assets.h` — generated, checked-in header with the three assets above as `const unsigned char foo[]` byte arrays (plus `foo_len`), produced via `xxd -i`. Regenerate after editing any of the three source assets:
  ```
  cd src && { xxd -i index.html; echo; xxd -i style.css; echo; xxd -i htmx.min.js.gz; } > web_assets.h
  sed -i '' 's/^unsigned char /const unsigned char /; s/^unsigned int \(.*\)_len/const unsigned int \1_len/' web_assets.h
  ```
  The `sed` step adds `const` so the arrays land in flash (`.rodata`) instead of being copied into RAM at boot (`xxd -i` doesn't emit `const` by default).
